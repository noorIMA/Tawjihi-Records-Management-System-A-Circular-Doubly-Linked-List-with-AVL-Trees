...
public class AVLAverage {
    ...
    public NodeAVL insert(NodeAVL root, Tawjihe average) {
        if (root == null) {
            return new NodeAVL(average);
        }

        if (average.compareTo(root.getData()) < 0) {
            root.left = insert(root.left, average);
        } else if (average.compareTo(root.getData()) > 0) {
            root.right = insert(root.right, average);
        } else {
        ...
        if (balance > 1 && average.compareTo(root.left.getData()) < 0) {
            return rightRotate(root);
        }

        if (balance < -1 && average.compareTo(root.right.getData()) > 0) {
    ...
    public NodeAVL delete(NodeAVL root, Tawjihe average) {
        if (root == null) {
            return root;
        }

        if (average.compareTo(root.getData()) < 0) {
            root.left = delete(root.left, average);
        } else if (average.compareTo(root.getData()) > 0) {
            root.right = delete(root.right, average);
        } else {
